...
def solve_sudoku(sudoku):
    ...
    def flip_matrix(_sudoku, _flip):
        if _flip == 'b' or _flip == 'd':
            _sudoku = np.flip(_sudoku, 1)
            # flip horizontally
        if _flip == 'c' or _flip == 'd':
            _sudoku = np.flip(_sudoku, 0)
            # flip vertically
        return _sudoku

    def unflip_matrix(_sudoku, flip):
        if flip == 'b':
            return np.flip(_sudoku, 1)
            # flip horizontally
        elif flip == 'd':
            stemp = np.flip(_sudoku, 1)
            return np.flip(stemp, 0)
        elif flip == 'c':
            return np.flip(_sudoku, 0)
        return _sudoku
